...
def get_fg(player_season_df, shot_type, shot_zone_basic, shot_area):
    ...
    vol = len(shots_df)
    #st.write(player_season_df['PLAYER_NAME'].values[0],shot_type, shot_zone_basic, shot_area)
    if vol > 5:
        xPTS = len(shots_df[shots_df['SHOT_MADE_FLAG']==1]) / vol
        #st.write(xPTS)
        return max(xPTS+vol/1000,.98)
    else:
...
def get_player_xpts(player_id,game_shotchart):
    ...
    #st.write('')
    #st.write(f"FINAL xPTS for {player_name} : {round(xPTS,1)}")
    #st.write('')
    return round(xPTS,1)
